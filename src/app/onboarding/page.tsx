  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors duration-200">
      ...
      <main className="flex-grow max-w-2xl mx-auto px-4 py-12 w-full flex items-center justify-center">
        <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white shadow-2xl w-full">
          <CardHeader className="border-b border-slate-200 dark:border-slate-800 text-center pb-6 bg-slate-50/50 dark:bg-slate-950/20">
            <CardTitle className="text-3xl font-extrabold tracking-tight flex items-center justify-center space-x-2 text-slate-900 dark:text-white">
              <Sparkles className="h-6 w-6 text-teal-600 dark:text-teal-400 animate-pulse" />
          ...
          <CardContent className="pt-8">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300 block mb-3 text-center">
                  Select Your Role *
                </label>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div
                    ...
                    className={`border-2 rounded-xl p-5 cursor-pointer flex flex-col items-center justify-center text-center transition-all duration-300 ${
                      role === "traveler"
                        ? "border-teal-500 bg-teal-50 dark:bg-teal-500/10 shadow-lg text-slate-900 dark:text-white"
                  ...
                  >
                    <User className={`h-8 w-8 mb-3 ${role === "traveler" ? "text-teal-600 dark:text-teal-400" : "text-slate-400 dark:text-slate-500"}`} />
                  ...
                  >
                    <Briefcase className={`h-8 w-8 mb-3 ${role === "planner" ? "text-teal-600 dark:text-teal-400" : "text-slate-400 dark:text-slate-500"}`} />
              ...
              {role === "traveler" && (
                <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800/50 animate-fadeIn">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center">
                      <Compass className="mr-2 h-4 w-4 text-teal-600 dark:text-teal-400" />
                      Preferred Travel Style *
                    </label>
                    <select
                      value={travelStyle}
                      onChange={(e) => setTravelStyle(e.target.value)}
                      className="w-full h-10 px-3 rounded-md bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-1 focus:ring-teal-500"
                    >
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center">
                      <Globe className="mr-2 h-4 w-4 text-teal-600 dark:text-teal-400" />
                      Home Base Location *
                    </label>
                    <Input
                      required
              ...
              {role === "planner" && (
                <div className="space-y-4 pt-4 border-t border-slate-200 dark:border-slate-800/50 animate-fadeIn">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-slate-700 dark:text-slate-300 flex items-center">
                      <Award className="mr-2 h-4 w-4 text-teal-600 dark:text-teal-400" />
                      Years of Experience *
                    </label>
                    <Input
                      type="number"
              ...
              <div className="pt-6 border-t border-slate-200 dark:border-slate-800 flex justify-end">
                <Button
                  type="submit"
                  disabled={loading}
                  className="bg-teal-500 hover:bg-teal-600 text-slate-950 font-bold px-6 py-2 rounded-md transition-colors cursor-pointer"
                >
