    ...
    <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100">
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 py-8 w-full">
        ...
        <div className="mb-8 text-center md:text-left">
          <h1 className="text-3xl font-extrabold text-white tracking-tight">Explore Travel Itineraries</h1>
          <p className="text-slate-400 mt-2">Discover curated trips planned by travelers and optimized by AI</p>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <form onSubmit={handleSearchSubmit} className="flex-1 relative">
            ...
            <Input
              ...
              className="pl-10 bg-slate-900 border-slate-800 text-white placeholder:text-slate-600 focus-visible:ring-teal-500 w-full"
            />
            <Button type="submit" className="absolute right-1 top-1 py-1 px-3 h-8 bg-teal-500 hover:bg-teal-600 text-slate-950 font-bold text-xs">
              Search
            </Button>
          </form>
          ...
          <div className="flex gap-3 justify-between">
            <Button
              ...
              className={`border-slate-800 hover:bg-slate-900 text-white ${showFilters ? "bg-slate-900 border-teal-500/50" : ""}`}
            >
              <SlidersHorizontal className="mr-2 h-4 w-4 text-teal-400" />
              Filters
            </Button>

            <div className="flex items-center space-x-2 bg-slate-900 px-3 py-1.5 rounded-lg border border-slate-800">
              <ArrowUpDown className="h-4 w-4 text-teal-400 shrink-0" />
              <select
                value={sort}
                onChange={(e) => {
              ...
              >
                ...
                <option value="cost_asc" className="bg-slate-900">Cost: Low to High</option>
                <option value="cost_desc" className="bg-slate-900">Cost: High to Low</option>
                <option value="duration_asc" className="bg-slate-900">Duration: Short to Long</option>
                <option value="duration_desc" className="bg-slate-900">Duration: Long to Short</option>
              </select>
            </div>
          </div>
        </div>
        ...
        <div className="flex overflow-x-auto pb-4 gap-2 no-scrollbar scroll-smooth mb-6 border-b border-slate-900">
          <Button
            onClick={() => {
              setCategory("");
              setPage(1);
            ...
            variant="ghost"
            className={`rounded-full shrink-0 border border-slate-800 px-4 py-1.5 text-xs font-semibold ${
              category === ""
        ...
        {showFilters && (
          <Card className="bg-slate-900 border-slate-800 text-white p-4 mb-6 transition-all duration-300 shadow-xl">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end">
              <div className="space-y-2">
                <label className="text-xs text-slate-400 font-semibold block">Min Budget ($)</label>
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-8 bg-slate-950 border-slate-800 text-white placeholder:text-slate-700"
                  />
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-xs text-slate-400 font-semibold block">Max Budget ($)</label>
                <div className="relative">
                  <DollarSign className="absolute left-2.5 top-2.5 h-4 w-4 text-slate-500" />
                  <Input
                    type="number"
              ...
              <div className="space-y-2">
                <label className="text-xs text-slate-400 font-semibold block">Duration (Days)</label>
                <div className="relative">
        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {Array.from({ length: 8 }).map((_, idx) => (
              <Card key={idx} className="bg-slate-900 border-slate-800 h-[380px] flex flex-col justify-between">
                <Skeleton className="h-44 w-full bg-slate-850 rounded-t-lg" />
                <div className="p-4 space-y-3 flex-grow">
                  <Skeleton className="h-4 w-1/4 bg-slate-800" />
                  <Skeleton className="h-5 w-3/4 bg-slate-800" />
                  <Skeleton className="h-10 w-full bg-slate-800" />
                </div>
                <CardFooter className="p-4 border-t border-slate-850 flex justify-between items-center">
          ...
          <div className="text-center py-12 bg-slate-900/40 rounded-xl border border-slate-800">
            <p className="text-rose-400 font-semibold mb-2">Error Loading Itineraries</p>
            <p className="text-slate-500 text-sm mb-4">Please make sure the backend server is running.</p>
            <Button onClick={() => refetch()} className="bg-teal-500 hover:bg-teal-600 text-slate-950">
              Try Again
            </Button>
          </div>
        ) : data?.itineraries.length === 0 ? (
          <div className="text-center py-16 bg-slate-900/40 rounded-xl border border-slate-800">
            <p className="text-slate-300 font-semibold text-lg mb-1">No Itineraries Found</p>
            <p className="text-slate-500 text-sm mb-6">Try adjusting your filters or search keywords.</p>
            <Button onClick={handleClearFilters} className="bg-teal-500 hover:bg-teal-600 text-slate-950 font-bold">
              Clear All Filters
            </Button>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {data?.itineraries.map((itinerary: ItineraryCardData) => (
              <Card
                key={itinerary._id}
                className="bg-slate-900 border-slate-800 overflow-hidden flex flex-col justify-between hover:border-slate-700 transition-all duration-300 group shadow-lg"
              >
                {/* Image & Tag */}
                ...
                {/* Footer Cost & Action */}
                <CardFooter className="p-4 border-t border-slate-800/60 flex items-center justify-between bg-slate-950/30">
                  <div>
        ...
        {!isLoading && data && data.pages > 1 && (
          <div className="flex justify-center items-center space-x-2 mt-10">
            <Button
              ...
              variant="outline"
              className="border-slate-800 hover:bg-slate-900 text-white disabled:opacity-50"
            >
              Previous
            </Button>
            {Array.from({ length: data.pages }).map((_, idx) => (
              <Button
                ...
                className={`w-9 h-9 font-bold text-xs rounded ${
                  page === idx + 1
